...
fn test_codec_trait_decodes_and_encodes_single_hex_byte() {
    ...
    let (decoded, consumed) = unsafe {
        Codec::decode_unchecked(&codec, b"Af", 0)
...
fn test_codec_trait_decodes_available_percent_byte() {
    ...
    let raw = unsafe { Codec::decode_unchecked(&codec, b"A", 0) }
...
fn test_codec_trait_decodes_and_encodes_form_urlencoded_byte() {
    ...
    let (decoded_escape, escape_consumed) = unsafe {
        Codec::decode_unchecked(&codec, b"%E4", 0)
            .expect("form escape should decode")
    };
    let (decoded_raw, raw_consumed) = unsafe {
        Codec::decode_unchecked(&codec, b"~", 0)
            .expect("form raw byte should decode")
    };
    ...
    let raw_written = unsafe {
        Codec::encode_unchecked(&codec, &b'~', &mut raw_output, 0)
...
fn test_codec_trait_decodes_available_form_urlencoded_byte() {
    ...
    let plus = unsafe { Codec::decode_unchecked(&codec, b"+", 0) }
...
fn test_codec_trait_decodes_available_c_string_literal_byte() {
    ...
    let terminated_hex =
        unsafe { Codec::decode_unchecked(&codec, br"\xAG", 0) }
    ...
    let terminated_octal =
        unsafe { Codec::decode_unchecked(&codec, br"\12G", 0) }
...
fn test_codec_trait_decodes_c_string_literal_escape_variants() {
    ...
    for (input, expected, expected_units) in cases {
        let (decoded, consumed) = unsafe {
            Codec::decode_unchecked(&codec, input, 0)
                .expect("C escape should decode")
        };
...
fn test_codec_trait_reports_c_string_literal_byte_errors() {
    ...
    let trailing_escape = unsafe { Codec::decode_unchecked(&codec, br"\", 0) }
        .expect_err("trailing escape should be incomplete");
    let short_hex_marker =
        unsafe { Codec::decode_unchecked(&codec, br"\x", 0) }
...
fn test_codec_trait_decodes_and_encodes_base64_quantum() {
    ...
    let (decoded, consumed) = unsafe {
        Codec::decode_unchecked(&codec, b"YWJj", 0)
...
fn test_codec_trait_decodes_and_encodes_url_safe_base64_quantum() {
    ...
    let (decoded, consumed) = unsafe {
        Codec::decode_unchecked(&codec, b"-__u", 0)
...
fn test_codec_trait_covers_base64_quantum_alphabet_and_errors() {
    ...
    let (decoded, _) =
        unsafe { Codec::decode_unchecked(&standard, b"++//", 0) }
            .expect("standard symbols should decode");
    assert_eq!([0xfb, 0xef, 0xff], decoded);

    let (decoded, _) =
        unsafe { Codec::decode_unchecked(&standard, b"0123", 0) }
