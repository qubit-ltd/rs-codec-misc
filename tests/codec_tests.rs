...
fn test_codec_trait_decodes_and_encodes_single_hex_byte() {
    ...
    let (decoded, consumed) = unsafe {
        Codec::decode_unchecked(&codec, b"Af", 0).expect("single hex byte should decode")
    };
    let written = unsafe {
        Codec::encode_unchecked(&codec, &0xaf, &mut output, 0)
...
fn test_codec_trait_reports_single_hex_byte_errors() {
    ...
    let high = unsafe { Codec::decode_unchecked(&codec, b"xf", 0) }
        .expect_err("invalid high hex digit should fail");
    let low = unsafe { Codec::decode_unchecked(&codec, b"fx", 0) }
...
fn test_codec_trait_decodes_and_encodes_percent_byte() {
    ...
    let (decoded_escape, escape_units) = unsafe {
        Codec::decode_unchecked(&codec, b"%E4", 0).expect("percent escape should decode")
    };
    ...
    let escaped_units = unsafe {
        Codec::encode_unchecked(&codec, &0xe4, &mut escaped, 0).expect("escaped byte should encode")
    };
    let unreserved_units = unsafe {
        Codec::encode_unchecked(&codec, &b'~', &mut raw, 0).expect("unreserved byte should encode")
    };
...
fn test_codec_trait_decodes_available_percent_byte() {
    ...
    let incomplete = unsafe { Codec::decode_unchecked(&codec, b"%E", 0) }
    ...
    let malformed = unsafe { Codec::decode_unchecked(&codec, b"%Ez", 0) }
...
fn test_codec_trait_decodes_and_encodes_form_urlencoded_byte() {
    ...
    let (decoded_plus, consumed) = unsafe {
        Codec::decode_unchecked(&codec, b"+", 0).expect("form plus should decode as space")
    };
    ...
    let plus_written = unsafe {
        Codec::encode_unchecked(&codec, &b' ', &mut plus_output, 0)
            .expect("space should encode as plus")
    };
    let raw_written = unsafe {
        Codec::encode_unchecked(&codec, &b'~', &mut raw_output, 0).expect("raw byte should encode")
    };
    let escaped_written = unsafe {
        Codec::encode_unchecked(&codec, &0xe4, &mut escaped_output, 0)
...
fn test_codec_trait_decodes_available_form_urlencoded_byte() {
    ...
    let plus =
        unsafe { Codec::decode_unchecked(&codec, b"+", 0) }.expect("plus should decode to space");
    let (decoded, consumed) = plus;
    assert_eq!(b' ', decoded);
    assert_eq!(1, consumed.get());

    let incomplete = unsafe { Codec::decode_unchecked(&codec, b"%", 0) }
...
fn test_codec_trait_decodes_available_c_string_literal_byte() {
    ...
    let raw = unsafe { Codec::decode_unchecked(&codec, br"A", 0) }
...
fn test_codec_trait_decodes_c_string_literal_escape_variants() {
    ...
    for (input, expected, expected_units) in cases {
        let (decoded, consumed) =
            unsafe { Codec::decode_unchecked(&codec, input, 0).expect("C escape should decode") };
...
fn test_codec_trait_reports_c_string_literal_byte_errors() {
    let codec = CStringLiteralCodec::new();
    let invalid_raw = unsafe { Codec::decode_unchecked(&codec, &[0xff], 0) }
        .expect_err("invalid raw byte should fail");
    let unsupported = unsafe { Codec::decode_unchecked(&codec, br"\z", 0) }
        .expect_err("unsupported escape should fail");
    let missing_hex = unsafe { Codec::decode_unchecked(&codec, br"\xz", 0) }
        .expect_err("missing hex digit should fail");
    let incomplete_universal = unsafe { Codec::decode_unchecked(&codec, br"\u12", 0) }
        .expect_err("incomplete universal escape should fail");
    let invalid_universal_digit = unsafe { Codec::decode_unchecked(&codec, br"\u00zz", 0) }
        .expect_err("invalid universal digit should fail");
    let oversized_universal = unsafe { Codec::decode_unchecked(&codec, br"\u0100", 0) }
...
fn test_codec_trait_encodes_c_string_literal_escape_variants() {
    ...
    for (byte, expected) in cases {
        ...
        let written = unsafe {
            Codec::encode_unchecked(&codec, byte, &mut output, 0)
...
fn test_codec_trait_decodes_and_encodes_base64_quantum() {
    ...
    let (decoded, consumed) = unsafe {
        Codec::decode_unchecked(&codec, b"YWJj", 0).expect("base64 quantum should decode")
    };
    let written = unsafe {
        Codec::encode_unchecked(&codec, b"abc", &mut output, 0)
...
fn test_codec_trait_decodes_and_encodes_url_safe_base64_quantum() {
    ...
    let (decoded, consumed) = unsafe {
        Codec::decode_unchecked(&codec, b"-__u", 0).expect("URL-safe quantum should decode")
    };
    let written = unsafe {
        Codec::encode_unchecked(&codec, &[0xfb, 0xff, 0xee], &mut output, 0)
...
fn test_codec_trait_covers_base64_quantum_alphabet_and_errors() {
    ...
    let (decoded, _) = unsafe { Codec::decode_unchecked(&standard, b"++//", 0) }
        .expect("standard symbols should decode");
    assert_eq!([0xfb, 0xef, 0xff], decoded);

    let (decoded, _) = unsafe { Codec::decode_unchecked(&standard, b"0123", 0) }
        .expect("digit symbols should decode");
    assert_eq!([0xd3, 0x5d, 0xb7], decoded);
    assert!(matches!(
        unsafe { Codec::decode_unchecked(&url_safe, b"@@@@", 0) }
...
fn test_value_traits_remain_convenience_layer() {
    ...
    let encoded = ValueEncoder::<[u8]>::encode(&codec, &[0x00, 0x7f, 0xff])
        .expect("hex value encode should work");
    let decoded =
        ValueDecoder::<str>::decode(&codec, &encoded).expect("hex value decode should work");
