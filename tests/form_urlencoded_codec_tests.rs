...
fn test_form_urlencoded_codec_uses_form_unescaped_set_for_low_level_encode() {
    ...
    let written = unsafe { Codec::encode(&mut codec, &b'*', &mut output, 0).expect("form '*' should be unescaped") };
    assert_eq!(1, written);
    assert_eq!(b'*', output[0]);

    let written = unsafe { Codec::encode(&mut codec, &b'~', &mut output, 0).expect("form '~' should be escaped") };
    assert_eq!(3, written);
