...
fn test_encoder_trait_dispatches_to_binary_codecs() {
    ...
    let hex = ValueEncoder::<[u8]>::encode(&mut HexCodec::upper(), &bytes).expect("hex should encode");
    let base64 = ValueEncoder::<[u8]>::encode(&mut Base64Codec::url_safe(), &bytes).expect("base64 should encode");
...
fn test_encoder_trait_dispatches_to_text_codecs() {
    let encoded = ValueEncoder::<str>::encode(&mut PercentCodec::new(), "a b/中").expect("percent should encode");
...
fn test_generic_value_encoder_example_accepts_mutable_codec() {
    fn encode_payload<C>(codec: &mut C, payload: &[u8]) -> Result<String, MiscCodecError>
    where
    ...
    let encoded = encode_payload(&mut codec, &[0xab, 0xcd]).expect("generic hex encoding should succeed");
