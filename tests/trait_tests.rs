...
use qubit_codec::{
    CodecTranscodeDecoder, CodecTranscodeEncoder, CodecValueEncoder, TranscodeDecoder,
    TranscodeEncoder, TranscodeError, TranscodeFailure,
    engine::{EncodeOutcome, TranscodeDecodeEngine, TranscodeEncodeEngine},
};
use qubit_codec_misc::{
    Base64Codec, CIntegerLiteralCodec, CStringLiteralCodec, FormUrlencodedCodec, HexByteCodec,
    HexCodec, MiscCodecError, PercentCodec, ValueDecoder, ValueEncoder,
};
...
fn test_core_codec_adapter_types_can_wrap_misc_codecs() {
    ...
    let encode_error = TranscodeError::<core::convert::Infallible, u8>::unencodable_value(2, 0xff);
    assert!(matches!(
        encode_error,
...
fn test_value_trait_map_error_methods_return_domain_errors() {
    ...
    assert_eq!(
        ...
        ValueEncoder::<[u8]>::map_error(&Base64Codec::standard(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueDecoder::<str>::map_error(&Base64Codec::standard(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueEncoder::<[u8]>::map_error(&CStringLiteralCodec::new(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueDecoder::<str>::map_error(&CStringLiteralCodec::new(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueDecoder::<str>::map_error(&CIntegerLiteralCodec::new(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueEncoder::<str>::map_error(&PercentCodec::new(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueDecoder::<str>::map_error(&PercentCodec::new(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueEncoder::<str>::map_error(&FormUrlencodedCodec::new(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueDecoder::<str>::map_error(&FormUrlencodedCodec::new(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueEncoder::<[u8]>::map_error(&HexCodec::new(), sample_error()).to_string(),
    );
    assert_eq!(
        "invalid test input: sample",
        ValueDecoder::<str>::map_error(&HexCodec::new(), sample_error()).to_string(),
    );
}
