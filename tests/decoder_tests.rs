...
use qubit_codec_misc::{Base64Codec, Base64ErrorKind, HexCodec, MiscCodecError};

#[test]
fn test_decoder_trait_dispatches_to_concrete_hex_decoder() {
    let mut codec = HexCodec::new().with_prefix("0x");

    let decoded = ValueDecoder::<str>::decode(&mut codec, "0x616263").expect("hex should decode");
...
fn test_decoder_trait_preserves_concrete_error_type() {
    let error = ValueDecoder::<str>::decode(&mut Base64Codec::standard(), "@@@")
