...
use qubit_codec_misc::{Base64Codec, MiscCodecError, ValueDecoder, ValueEncoder};
...
fn test_base64_codec_can_be_used_through_traits() {
    let codec = Base64Codec::standard();
    let encoded =
        ValueEncoder::<[u8]>::encode(&codec, b"abc").expect("base64 encode should succeed");
    let decoded =
        ValueDecoder::<str>::decode(&codec, &encoded).expect("base64 decode should succeed");
