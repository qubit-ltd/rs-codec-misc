...
impl Codec for PercentCodec {
    ...
    unsafe fn encode(
        ...
        let written = percent_encode_byte(*value, output, output_index, PercentEncodingMode::Rfc3986);
    ...
    unsafe fn decode_eof(
        ...
        let (value, consumed) = percent_decode_byte_eof(input, input_index, false).map_err(|error| {
            map_misc_decode_failure_with_consumed(error, percent_invalid_consumed(input, input_index))
        })?;
        debug_assert!(consumed > 0);
        let consumed = nonzero(consumed);
        Ok((value, consumed))
...
pub(crate) fn percent_encode_bytes(bytes: &[u8], mode: PercentEncodingMode) -> String {
    let capacity = bytes.iter().map(|byte| percent_encode_len(*byte, mode)).sum();
...
#[inline]
pub(crate) fn percent_encode_byte(byte: u8, output: &mut [u8], index: usize, mode: PercentEncodingMode) -> usize {
    if mode.is_space_plus(byte) {
        output[index] = b'+';
        return 1;
    }
...
#[inline]
pub(crate) fn percent_decode_byte(input: &[u8], index: usize, plus_as_space: bool) -> Result<(u8, usize), ParseError> {
    ...
    match input[index] {
        b'%' => {
            ...
            let high = percent_hex_value(high_byte).ok_or_else(|| invalid_percent_escape(input, index))?;
            let low = percent_hex_value(low_byte).ok_or_else(|| invalid_percent_escape(input, index))?;
            Ok(((high << 4) | low, 3))
        }
...
#[inline]
fn percent_decode_byte_eof(input: &[u8], index: usize, plus_as_space: bool) -> Result<(u8, usize), MiscCodecError> {
    percent_decode_byte(input, index, plus_as_space).map_err(|error| percent_parse_error_to_misc(error, input, index))
}
